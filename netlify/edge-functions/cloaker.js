...
// REDIRECT PARA: https://teste-e.protocolovital4f.online
...
export default async (request, context) => {
  try {
    ...
    // ✅ ALTERAÇÃO: NOVO DOMÍNIO DE DESTINO
    const targetUrl = `https://teste-e.protocolovital4f.online${url.pathname}${url.search}`;
    ...
    // Failsafe: em caso de erro, redirect direto para o novo domínio
    const targetUrl = `https://teste-e.protocolovital4f.online${new URL(request.url).pathname}${new URL(request.url).search}`;
